    ...
    public class EcdsaKey : Key, IDisposable
    {
        ...
        public override BigInteger[] Public
        {
            get
            {
                ...
                return new[]
                {
                    new BigInteger(curve.Reverse()), new BigInteger(q.Reverse())
                };
            }
            set
            {
        ...
        public EcdsaKey(byte[] data)
        {
            ...
            // PrivateKey
            byte[] privatekey = der.ReadOctetString().TrimLeadingZeros();
            ...
            byte[] construct = der.ReadBytes(der.ReadLength()); // object length

            // curve OID
            var curve_der = new DerData(construct, true);
            byte[] curve = curve_der.ReadObject();
            ...
            byte[] pubkey = pubkey_der.ReadBitString().TrimLeadingZeros();
